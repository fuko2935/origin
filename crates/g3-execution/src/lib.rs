...
impl CodeExecutor {
    ...
    /// Execute bash command with streaming output and idle timeout detection.
    /// If no output is produced for `idle_timeout_secs`, the command is killed.
    pub async fn execute_bash_streaming_with_idle_timeout<R: OutputReceiver>(
        &self,
        code: &str,
        receiver: &R,
        working_dir: Option<&str>,
        ...
        debug!("execute_bash_streaming_with_idle_timeout: code='{}', timeout={}s", code, idle_timeout_secs);
        ...
        loop {
            ...
            let timer = sleep(Duration::from_secs(idle_timeout_secs));
            ...
            tokio::select! {
                ...
                _ = timer => {
                    ...
                    let error_message = format!(
                        "IDLE TIMEOUT: Command produced no output for {} seconds.\n\n\
                         The command may be waiting for user input or is stuck.\n\n\
                         Hint: Retry with a longer timeout parameter if needed.",
                        idle_timeout_secs
                    );
...
mod idle_timeout_tests {
    ...
    impl OutputReceiver for TestReceiver {
        fn on_output_line(&self, line: &str) {
    ...
    #[tokio::test]
    async fn test_idle_timeout_fast_command() {
        let executor = CodeExecutor::new();
        let receiver = TestReceiver::new();

        let result = executor
            .execute_bash_streaming_with_idle_timeout("echo 'hello world'", &receiver, None, 5)
            .await
            .unwrap();
    ...
    async fn test_idle_timeout_triggers() {
        ...
        // sleep 10 seconds but timeout after 1 second
        let result = executor
            .execute_bash_streaming_with_idle_timeout("sleep 10", &receiver, None, 1)
            .await
            .unwrap();

        assert!(!result.success);
        assert_eq!(result.exit_code, 124);
        assert!(result.stderr.contains("IDLE TIMEOUT"));
    }

    #[tokio::test]
    async fn test_idle_timeout_output_resets_timer() {
        let executor = CodeExecutor::new();
        let receiver = TestReceiver::new();

        // This command outputs every 0.5s for 2s total, timeout is 2s
        // Should complete successfully because output keeps resetting timer
        let result = executor
